SELECT rate.overall_rating_plays, homeplay.home_plays
FROM
(SELECT COUNT(MM.id) as home_plays
FROM match_info MM
WHERE home_team_score> away_team_score) as homeplay,
(SELECT COUNT(Home.id) as overall_rating_plays
...
WHERE Home.id = Away.id AND 
	((Home.home_overall_rating>Away.away_overall_rating AND Home.home_score>Home.away_score) OR
	(Home.home_overall_rating<Away.away_overall_rating AND Home.home_score<Home.away_score))
ORDER BY Home.id) as rate
